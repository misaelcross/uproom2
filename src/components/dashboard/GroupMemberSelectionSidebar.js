...
const GroupMemberSelectionSidebar = ({ isOpen, onClose, onContinue, selectedMembers = [], onMemberToggle }) => {
  ...
  const isSelected = (employeeId) => {
    return selectedMembers.some(member => member.id === employeeId);
  };

  const handleMemberClick = (employee) => {
    onMemberToggle(employee);
  };
  ...
  return (
    <div className="h-full flex flex-col border border-neutral-700 rounded-lg">
        ...
        <div className="flex items-center justify-between p-4 border-b border-neutral-700 flex-shrink-0">
          <h2 className="text-lg font-semibold text-white">Select Group Members</h2>
          <button
            onClick={onClose}
        ...
        {selectedMembers.length > 0 && (
          <div className="px-4 py-2 bg-neutral-800/50 border-b border-neutral-700 flex-shrink-0">
            ...
            <div className="p-2">
              {filteredEmployees.map((employee) => {
                ...
                return (
                  <div
                    key={employee.id}
                    onClick={() => handleMemberClick(employee)}
...
export default GroupMemberSelectionSidebar;